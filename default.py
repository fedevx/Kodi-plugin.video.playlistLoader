...
def AddNewList():
    xbmc.log("Tracker AddNewList", xbmc.LOGINFO)
...
def AddDir(name, url, mode, iconimage='', logos='', epg='', index=-1, move=0, uuid='0', isFolder=True, IsPlayable=False, background=None, cacheMin='0', plot="", fanart="", addToVdir=True):
    ...
    if mode == 1 or mode == 2:
        ...
        if mode == 2 and not url.endswith('.plx'):
            ...
            items.append((getLocaleString(30103), 'RunPlugin({0}?index={1}&mode=29&uuid={2})'.format(sys.argv[0], index, uuid)))
...
def AddNewDirectory():
    xbmc.log("Tracker: AddNewDirectory", xbmc.LOGINFO)
...
if mode == 0:
    ...
    ChangeChoice(uuid, playlistsFile, "epg", 30104, 30105, 30106, 30105, 30106, 30021, 0)    
